...
/**
 * Tests the {@link GenericRowRecordReader}
 */
public class GenericRowRecordExtractorTest extends AbstractRecordExtractorTest {

  private final File _dataFile = new File(_tempDir, "events.ser");
  ...
  protected RecordReader createRecordReader(Set<String> fieldsToRead)
      throws IOException {
    GenericRowRecordReader recordReader = new GenericRowRecordReader();
    recordReader.init(_dataFile, fieldsToRead, null);
    return recordReader;
  }

  /**
   * Create a GenericRow serialized input file using the input records
   */
  @Override
  protected void createInputFile()
      throws IOException {
    try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(_dataFile))) {
      for (Map<String, Object> inputRecord : _inputRecords) {
        GenericRow genericRow = new GenericRow();
        for (String key : inputRecord.keySet()) {
          genericRow.putValue(key, inputRecord.get(key));
        }
        oos.writeObject(genericRow);
      }
    }
  }
